...
class Node(ClientNode):
    ...
    def __init__(self, host, input_attributes=None, output_attributes=None, is_first=False):
        ...
        fmu_name=("..\wrappers\IBPSA_Fluid_FixedResistances_Examples_PlugFlowPipe.fmu")
        # Load model
        self.pipe = load_fmu(fmu_name)
    ...
    def step(self, current_time, time_step):
        ...
        res = self.pipe.simulate(final_time=3000)
        ...
        for key in self.output_attributes:
            self.update_attribute(key, getattr(self, res(key)))

        # Save output attributes and corresponding simulation time step to Redis DB
        for key in self.output_attributes:
            self.redis.rpush('OUT||' + self.name + '||' + key, getattr(self, res(key)))
